...
public class ArraysHandler {
	...
	// Find Index of 0 to be replaced with 1 to get longest continuous sequence
	...
	// Find length of the longest consecutive path from a given starting
	// character
	...
	// Count frequencies of all elements in array in O(1) extra space and O(n)
	// time
	public static void countFreq(int[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			System.out.println("value=" + (i + 1) + " freq=" + Math.abs(arr[i]));
		}
	}

	public static void countFreqSolB(int[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			System.out.println("value=" + (i + 1) + " freq=" + arr[i] / n);
		}
	...
	//Auxiliary Space : O(1)
	public static int countPairsSolA(int[] arr, int sum) {
	...
	//Time Complexity : O(n)
	//Auxiliary Space : O(n)
	public static int countPairsSolB(int[] arr, int sum) {
		
		Map<Integer,Integer> storeNumbersFreq = null;
		Integer key = null;
		int twice_count = 0;
		...
		storeNumbersFreq = new HashMap<Integer,Integer>();
		
		for (int i = 0; i < arr.length; i++) {
			key = new Integer(arr[i]);
			if(storeNumbersFreq.containsKey(key)){
				storeNumbersFreq.put(key,storeNumbersFreq.get(key) + 1);
			}else{
				storeNumbersFreq.put(key,1);
			}
		}
		//{1, 5, 7, -1,1,5,5,3,3,3} {1,1,1,1} sum=6
		for (int i = 0; i < arr.length; i++) {
			if(storeNumbersFreq.containsKey(sum - arr[i])){
				twice_count += storeNumbersFreq.get(sum - arr[i]);
